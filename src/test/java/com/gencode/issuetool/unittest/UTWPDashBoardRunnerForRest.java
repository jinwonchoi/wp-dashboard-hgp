...
public class UTWPDashBoardRunnerForRest {
	...
	/**
	 * test title:로그인한 후 정상인증처리하고 token가져오기 description:
	 * 
	 * test id: test step: 1. admin계정으로 로그인 2. token으로 사용자목록 가져오기
	 * 
	...
	public void runLoginAndGetUserList() {
		try {
			...
			Type type = new TypeToken<ResultObj<AuthUserInfo>>() {
			}.getType();
			ResultObj<AuthUserInfo> result = restClient.callJsonHttp(testData.URL + "/auth/login",
					testData.getUserInfoAdmin(), type);
			...
			Type type2 = new TypeToken<ResultObj<List<UserInfo>>>() {
			}.getType();
			RestClient<List<UserInfo>> restClient2 = new RestClient<List<UserInfo>>(token);
			ResultObj<List<UserInfo>> list = (ResultObj<List<UserInfo>>) restClient2
					.callJsonHttp(testData.URL + "/user/list", "", type2);
...
//    		ResultObj<List<UserInfo>> result = (ResultObj<List<UserInfo>>) (new RestClient<List<UserInfo>>(token)).callJsonHttp(url, map, type);
	 ...
	 */
	@Test
	public void runLoginAndGetDsMain() {
		try {
			RestClient<AuthUserInfo> restClient = new RestClient<AuthUserInfo>();
			Type type = new TypeToken<ResultObj<AuthUserInfo>>() {
			}.getType();
...
//    		ResultObj<List<UserInfo>> result = (ResultObj<List<UserInfo>>) (new RestClient<List<UserInfo>>(token)).callJsonHttp(url, map, type);
	...
	@Test
	public void runLoginAndGetDsIot() {
		try {
			RestClient<AuthUserInfo> restClient = new RestClient<AuthUserInfo>();
			Type type = new TypeToken<ResultObj<AuthUserInfo>>() {
			}.getType();
...
//    		ResultObj<List<UserInfo>> result = (ResultObj<List<UserInfo>>) (new RestClient<List<UserInfo>>(token)).callJsonHttp(url, map, type);
	...
	/**
	 * test title:Admin이 사용자 등록 description:
	 * 
	 * test id: test step: 1. admin계정으로 로그인 2. 사번/팀/법인으로 사용자등록
	 * 
	 */
	...
	/**
	 * test title:비번초기화상태로 사용자로그인 description: 사용자가 비번초기화관련 인증 및 비번변경처리
	 * 
	 * test id: test step: 1. agent04로 로그인 ==> 비번변경오류코드 리턴 2. 비번변경 등록 => 인증번호리턴 3.
	 * 인증번호 등록 => 로그인 성공
	 */

	/**
	 * test title:비번변경상태로 사용자로그인 description: 사용자가 비번초기화관련 인증 및 비번변경처리
	 * 
	 * test id: test step: 1. agent04로 로그인 ==> 인증번호리턴 3. 인증번호 등록 => 로그인 성공
	 */

	/**
	 * test title:loginId찾기 description: 이메일/폰번호로 loginId요청
	 * 
	 * test id: test step: 1. 이메일로 요청 2. 문자로 요청
	 */

	/**
	 * test title:사용자 프로파일 변경 description:
	 * 
	 * test id: test step: 1. 이메일로 요청 2. 문자로 요청
	 */

	/**
	 * test title:로그인한 후 공지사항 등록 수정 조회 삭제 description:
	 * 
	 * test id: test step: 1. admin계정으로 로그인 2. 등록 1 3. Get 4. 수정 5. Get 6. 등록 2 7.
	 * 목록 조회 8. 삭제 2 9. 목록 조회
	 * 
	 * @throws InterruptedException
	 * @throws ExecutionException
	 * @throws TimeoutException
	...
	public void runLoginAndDoCrudNoticeBoard() {
		try {
			...
			// * 8. 삭제 2
			// String resultDelete = _callDeleteNotice("8. 삭제 2",
			// true,resultNotice2.getId());
			// * 9. 목록 조회
	...
	@Test
	public void runLoginAndGetDashboard() {
		try {
			RestClient<AuthUserInfo> restClient = new RestClient<AuthUserInfo>();
			Type type = new TypeToken<ResultObj<AuthUserInfo>>() {
			}.getType();
			...
			RestClient<String> restClient2 = new RestClient<String>(token);
			ResultObj<String> list = (ResultObj<String>) restClient2
					.callJsonHttp(testData.URL +"/pwrplant/main-facil", "", type2);
			logger.assertResult("Get Dashboard", "token으로 사용자목록 가져오기", list.getItem()!=null);
...
//    		ResultObj<List<UserInfo>> result = (ResultObj<List<UserInfo>>) (new RestClient<List<UserInfo>>(token)).callJsonHttp(url, map, type);
	...
	List<UserInfo> _callAgentList(String testTitle, boolean flag) {
		String url = testData.URL + "/user/search";
		Type type = new TypeToken<ResultObj<List<UserInfo>>>() {
		}.getType();
		Map<String, String> map = new HashMap<String, String>();
		map.put("role", "O");
		ResultObj<List<UserInfo>> result = (ResultObj<List<UserInfo>>) (new RestClient<List<UserInfo>>(token))
				.callJsonHttp(url, map, type);
	...
	NoticeBoard _callRegisterNotice(String testTitle, boolean flag, NoticeBoard notice) {
		...
		Type type = new TypeToken<ResultObj<NoticeBoard>>() {
		}.getType();
		ResultObj<NoticeBoard> result = (ResultObj<NoticeBoard>) (new RestClient<NoticeBoard>(token)).callJsonHttp(url,
				notice, type);
	...
	String _callUpdateNotice(String testTitle, boolean flag, NoticeBoard notice) {
		...
		Type type = new TypeToken<ResultObj<String>>() {
		}.getType();
	...
	PageResultObj<List<NoticeBoardEx>> _callListPageNotice(String testTitle, boolean flag) {
		...
		Type type = new TypeToken<PageResultObj<List<NoticeBoardEx>>>() {
		}.getType();
		Map<String, String> map = new HashMap<String, String>();

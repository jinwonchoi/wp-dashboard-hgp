...
@Component
public class PrtFileInfoDaoImpl extends PrtAbstractDaoImpl implements PrtFileInfoDao {

	//final String queryStr = " select id, file_name, file_size, file_ext, mime_type, storage_dir, file_org_name, file_desc, tag_info, register_id, delete_yn, updated_dtm, created_dtm FROM proto_file_info where delete_yn='N' ";
	final String queryStr = "select fi.id, fi.file_name,fi.file_type, fi.file_size, fi.file_ext, fi.mime_type, fi.storage_dir, fi.file_org_name, fi.file_desc, fi.tag_info, fi.status, fi.register_id, fi.delete_yn, fi.updated_dtm, fi.created_dtm "  
			...
			+" from proto_file_info fi " 
			+" left outer join proto_user_info ui on (fi.register_id = ui.id) where fi.delete_yn='N' ";
	final String queryStr2 = "select fi.id, fi.file_name, fi.file_type, fi.file_size, fi.file_ext, fi.mime_type, fi.storage_dir, fi.file_org_name, fi.file_desc, fi.tag_info, fi.status, fi.register_id, fi.delete_yn, fi.updated_dtm, fi.created_dtm "
			+ ObjMapper.USER_INFO_LIST
			+" from proto_file_info fi"
			+" left outer join proto_user_info ui on (fi.register_id = ui.id) "
			+", proto_file_reference fr where fi.id = fr.file_id and fr.ref_id = :refId and fi.delete_yn = 'N' and fr.delete_yn = 'N' ";
	
	public PrtFileInfoDaoImpl(JdbcTemplate jdbcTemplate, NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
	...
	public long register(FileInfo t) {
		...
		namedParameterJdbcTemplate.update("INSERT INTO proto_file_info(file_name, file_type, file_size, file_ext, mime_type, storage_dir, file_org_name, file_desc, tag_info, status, register_id, updated_dtm, created_dtm) " + 
	...
	public long delete(long id) {
		return namedParameterJdbcTemplate.update("UPDATE proto_file_info SET delete_yn = 'Y',updated_dtm =NOW(3)  where id = :id",
			new MapSqlParameterSource("id", id));
	}

	@Override
	public void forceDelete(long id) {
		namedParameterJdbcTemplate.update("DELETE FROM proto_file_info where id = :id",
			new MapSqlParameterSource("id", id));
	}
	
	@Override
	public void completeUpload(long id) {
		namedParameterJdbcTemplate.update("UPDATE proto_file_info SET status = 'C',updated_dtm =NOW(3)  where id = :id",
				new MapSqlParameterSource("id", id));
	}
	
	@Override
	public long update(FileInfo t) {
		return namedParameterJdbcTemplate.update("UPDATE proto_file_info SET " +
				"file_name =:fileName,"+ 
				"file_type =:fileType,"+ 
				"file_size =:fileSize,"+
				"file_ext =:fileExt, "+

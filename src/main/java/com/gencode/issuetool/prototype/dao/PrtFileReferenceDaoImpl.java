...
@Component
public class PrtFileReferenceDaoImpl extends PrtAbstractDaoImpl implements PrtFileReferenceDao {
	...
	final String queryStr = "select fr.id, fr.file_id, fr.ref_id, fr.ref_type, fr.ref_desc, fr.storage_dir, fr.tag_info, fr.status, fr.register_id, fr.delete_yn, fr.updated_dtm, fr.created_dtm "  
			...
			+" from proto_file_reference fr " 
			+" left outer join proto_user_info ui on (fr.register_id = ui.id) where fr.delete_yn='N' ";
	
	public PrtFileReferenceDaoImpl(JdbcTemplate jdbcTemplate, NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
	...
	public long register(FileReference t) {
		...
		namedParameterJdbcTemplate.update("INSERT INTO proto_file_reference(file_id, ref_id, ref_type, ref_desc, storage_dir, tag_info, status, register_id, updated_dtm, created_dtm) " + 
	...
	public Optional<FileReference> load(long id) {
		FileReference t = DataAccessUtils.singleResult(namedParameterJdbcTemplate.query(
				...
				new MapSqlParameterSource("id",id ), (resultSet,i)->{
					return ObjMapper.toFileReference(resultSet);
				}));
	...
	public long delete(long id) {
		return namedParameterJdbcTemplate.update("UPDATE proto_file_reference SET delete_yn = 'Y',updated_dtm =NOW(3) where id = :id",
			new MapSqlParameterSource("id", id));
	}
	@Override
	public long delete(FileReference t) {
		return namedParameterJdbcTemplate.update("UPDATE proto_file_reference SET delete_yn = 'Y',updated_dtm =NOW(3) where ref_id = :refId and file_id = :fileId"
				,new BeanPropertySqlParameterSource(t));
	}
	@Override
	public long forceDelete(long id) {
		return namedParameterJdbcTemplate.update("DELETE FROM proto_file_reference where id = :id",
			new MapSqlParameterSource("id", id));
	}
	@Override
	public long forceDelete(FileReference t) {
		return namedParameterJdbcTemplate.update("DELETE FROM proto_file_reference where ref_id = :refId and file_id = :fileId"
				,new BeanPropertySqlParameterSource(t));
	}
	
	@Override
	public long update(FileReference t) {
		return namedParameterJdbcTemplate.update("UPDATE proto_file_reference SET " +
				"file_id = :fileId,"+
				"ref_id = :refId,"+
				"ref_type = :refType,"+
				"ref_desc = :refDesc,"+

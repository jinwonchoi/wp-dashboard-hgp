...
@Component("PrtUserInfoDao")
public class PrtUserInfoDaoImpl extends PrtAbstractDaoImpl implements PrtUserInfoDao {
	...
	public PrtUserInfoDaoImpl(JdbcTemplate jdbcTemplate, NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
	...
	public long register(UserInfo t) {
		...
		namedParameterJdbcTemplate.update("INSERT INTO proto_user_info(login_id,level,role,position,group_id,user_name,user_email,mobile_no,phone_no,passwd,auth_key,use_yn,passwd_update_date,user_profile,user_status,profile_url,avatar_url,access_token,notice_id,registered_dtm,updated_dtm,created_dtm)\r\n" + 
	...
	public Optional<UserInfo> login(String loginId) {
		UserInfo t = DataAccessUtils.singleResult(namedParameterJdbcTemplate.query(
				"select "+fields+" from proto_user_info where use_yn = 'Y' and user_status in ('R', 'C', 'A') and login_id = :loginId",
	...
	public void activate(UserInfo userInfo) {
		namedParameterJdbcTemplate.update("UPDATE proto_user_info SET user_status = 'A'" + 
	...
	public Optional<UserInfo> load(long id) {
		UserInfo t = DataAccessUtils.singleResult(namedParameterJdbcTemplate.query(
				"select "+fields+" from proto_user_info where id = :id",
	...
	public long forceDelete(long id) {
		return namedParameterJdbcTemplate.update("DELETE FROM proto_user_info where id = :id",
				new MapSqlParameterSource("id", id));
	}

	@Override
	public long delete(long id) {
		return namedParameterJdbcTemplate.update("UPDATE proto_user_info SET user_status = 'D', use_yn='N'" + 
				" WHERE id=:id"
				,new MapSqlParameterSource("id",id ));
	}
	...
	public long update(UserInfo t) {
		return namedParameterJdbcTemplate.update("UPDATE proto_user_info SET " +
				"login_id			=:loginId,"+
				"level				=:level,"+
				"role				=:role,"+
				"position			=:position,"+
	...
	public Optional<List<UserInfo>> loadAll() {
		List<UserInfo> list = jdbcTemplate.query(
				"select "+fields+" from proto_user_info where 1=1", (resultSet, i) -> {
            return toUserInfo(resultSet);
        });
	...
	public Optional<List<UserInfo>> searchExtrict(Map<String, String> map) {
		...
		List<UserInfo> t = namedParameterJdbcTemplate.query
				("select "+fields+" from proto_user_info where 1=1"
	...
	public Optional<PageResultObj<List<UserInfo>>> search(PageRequest req) {
		String queryStr = "select "+fields+" from proto_user_info where 1=1";

...
public interface IotMainHistStatDao extends Dao<IotMainHistStat> {
	int[] register(List<IotMainHistStat> t);
	/** 4. 분단위 등록
		4.1. 분단위 등록시 기존등록 삭제
		hst테이블=>stat테이블
	 */
	...
	*/
	 ...
	 * @return
	 */
	Optional<List<IotMainHistStat>> getStatsCnt(PageRequest req);
	
	Optional<List<IotMainHistStat>> getStatsCntOvervalue(PageRequest req);
	/**
	 * 기간별 최근 30건 
	 * @param map
	 * @return
	 */
	Optional<List<IotMainHistStat>> getRealtimeChartData(PageRequest req);
}

...
public interface TagDataHistStatDao extends Dao<TagDataHistStat> {
	int[] register(List<TagDataHistStat> t);
	/** 4. 분단위 등록
		4.1. 분단위 등록시 기존등록 삭제
		hst테이블=>stat테이블
	 */
	...
	*/
	 ...
	 * @return
	 */
	Optional<List<TagDataHistStat>> getStatsCnt(PageRequest req);
	/**
	 * 기간별 최근 30건 
	 * @param map
	 * @return
	 */
	Optional<List<TagDataHistStat>> getRealtimeChartData(PageRequest req);
}
